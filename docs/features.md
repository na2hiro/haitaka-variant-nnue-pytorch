...
Composite feature sets are first-class: a feature set can concatenate a factorized
block with extra sparse real blocks. In that case only the virtual factors of the
factorized blocks are coalesced on `.nnue` export; appended real-only blocks are
stored as-is in the exported geometry.
...
2. When resuming training from a .pt model - `--features` specifies the feature set to use for learning. If the feature set specified doesn't match the feature set from the .pt model a conversion is attempted. Conversion works when the old and new feature sets have matching real-feature ranges block-by-block. Real weights are copied into the new geometry, while any newly introduced virtual rows are zero-initialized and PSQT rows are reinitialized from the destination feature set.
...
4. When converting .pt to .nnue - `--features` is ignored, the `feature_set` from the saved model is used. Virtual factors are coalesced when writing the `.nnue` file, while extra real-only blocks in composite feature sets are preserved as independent ranges.
5. When converting .nnue to .pt - `--features` specifies the features in the `.nnue` file. The resulting `.pt` model has the same feature_set. For composite feature sets the reader reconstructs the full training tensor by expanding the stored real rows back into their block-local ranges. Note that when resuming training this model can be converted to a compatible feature_set, see point 2.
